  ...
  if (in < N ){
    ...
    if (u+innw >= 0 && u+innw < W && v+innh >= 0 && v+innh < H){
      if (grid_valid[ib][0][v+innh][u+innw] > 0){
        ...
        if (return_pdf){
          y[0][blockIdx.y][in] = det_neghalf * exp(-0.5 * (dx * dx * inv_xx + dy * dy * inv_yy + 2 * dx * dy * inv_xy)) * weight;   // pdf of multivariate normal
        }
        else{
          y[0][blockIdx.y][in] = exp(-0.5 * (dx * dx * inv_xx + dy * dy * inv_yy + 2 * dx * dy * inv_xy)) * weight;   // pdf of multivariate normal
        }
    ...
    if (in < N ){
      ...
      if (u+innw >= 0 && u+innw < W && v+innh >= 0 && v+innh < H){
        if (grid_valid[ib][0][v+innh][u+innw] > 0){
          ...
          if (return_pdf){
            res_dsigma_x = res_dsigma_x + res_exp * weight * ddet_neghalf_ddet * ddet_dsigma_x;
            res_dsigma_y = res_dsigma_y + res_exp * weight * ddet_neghalf_ddet * ddet_dsigma_y;
            res_drho_xy = res_drho_xy + res_exp * weight * ddet_neghalf_ddet * ddet_drho_xy;
            res_dweight = res_dweight * det_neghalf;
          }
