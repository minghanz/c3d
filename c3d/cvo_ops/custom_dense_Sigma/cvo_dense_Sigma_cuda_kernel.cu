    ...
    if (in < N ){
      ...
      if (u+innw >= 0 && u+innw < W && v+innh >= 0 && v+innh < H){
        if (grid_valid[ib][0][v+innh][u+innw] > 0){
          ...
          float res_exp = exp(-0.5 * (dx * dx * inv_xx + dy * dy * inv_yy + 2 * dx * dy * inv_xy) );
          float res = det_neghalf * res_exp * weight;

          float res_dx_pts = res * ( inv_xx * dx + inv_xy * dy );
          float res_dy_pts = res * ( inv_xy * dx + inv_yy * dy );
          ...
          float res_dinv_xx = res * (-0.5 * dx * dx);
          float res_dinv_yy = res * (-0.5 * dy * dy);
          float res_dinv_xy = res * (- dx * dy);
