...
def eval_preprocess(depth_pred, depth_gt, d_min, d_max, shape_unify=None, eval_crop=None):
    ...
    mask_value = np.logical_and(depth_gt >= d_min, depth_gt <= d_max)
    if eval_crop is None:
        mask_crop = np.ones_like(depth_gt, dtype=np.bool)
    elif eval_crop == "garg_crop":
        mask_crop = np.zeros_like(depth_gt, dtype=np.bool)
